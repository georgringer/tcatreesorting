...
namespace GeorgRinger\Tcatreesorting\Xclass;
...
class TreeElement extends \TYPO3\CMS\Backend\Form\Element\TreeElement {
	...
	public function renderField($table, $field, $row, &$PA, $config, $possibleSelectboxItems, $noMatchLabel) {
		...
		$pageRenderer->addJsFile(ExtensionManagementUtility::extRelPath('tcatreesorting') . 'Resources/Public/JavaScript/tree.js');
	...
	protected function getGroup($PA, $id) {
		$PA['fieldConf']['config']['internal_type'] = 'db';
		$PA['fieldConf']['config']['allowed'] = 'tx_news_domain_model_category';
		$PA['fieldConf']['config']['disable_controls'] = 'browser';
		$code = $this->tceForms->getSingleField_typeGroup('tx_news_domain_model_news', 'categories', $PA['row'], $PA);
		...
		$newCode = substr($newCode, 0, $pos + strlen($search)) . LF . '<input id="treeinput' . $id . '" class="treeRecord" type="hidden" name="' . htmlspecialchars($PA['itemFormElName']) . '"  value="' . htmlspecialchars($PA['itemFormElValue']) . '" />';
